...
class UserOptimizedTransformerTests(unittest.TestCase):
    ...
    def test_external_final_norm_dispatch_is_case8_only(self) -> None:
        case8 = TransformerConfig(64, 128, 1024, 4, 1024, 4, True)
        selected = UserOptimizedTransformer(case8)
        self.assertTrue(selected._external_final_norm_eligible())
        self.assertIs(
            selected.forward.__func__,
            UserOptimizedTransformer._forward_case8_graph,
        )
    ...
    def test_case8_external_final_norm_returns_independent_outputs(self) -> None:
        case8 = TransformerConfig(64, 128, 1024, 4, 1024, 4, True)
        optimized = UserOptimizedTransformer(case8).eval()
    ...
    def test_case8_graph_boundary_preserves_final_norm_and_mask_order(self) -> None:
        config = TransformerConfig(1, 8, 16, 4, 32, 1, True)
        optimized = UserOptimizedTransformer(config).eval()
