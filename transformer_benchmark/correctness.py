"""Numerical correctness checks for Transformer implementations."""
...
def compare_outputs(
    ...
    # Exact official rule. torch.isclose uses atol + rtol * abs(ref), which is
    # more permissive and does not preserve the strict OR semantics.
    ...
    if reference.ndim == 0:
        ...
        failed_feature_dims = (
            torch.nonzero(failed_mask, as_tuple=False).flatten().tolist()
        )
    else:
...
def run_accuracy_tests(
    ...
    total_failed = 0
    total_elements = 0
    trial_results: List[AccuracyResult] = []
